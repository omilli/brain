...
A markdown knowledge base at `<repo>/memory/` records verified decisions and confirmed facts/corrections so prior learning is recallable instead of re-derived. Concept files (`entries/*.md`) are the canonical source; `index.md` is a derived directory-listing regenerated by `python3 .agents/skills/memory/memory.py rebuild` (never hand-edited); `archive/` holds retired concepts. Entries load only on trigger — never every run.

- **Read before acting:** before editing a shared/public symbol, changing an established decision, or re-trusting a rule that failed last run — grep `<repo>/memory/index.md` for the keyword, then read the matched concept. Re-verify before trusting: run `python3 .agents/skills/memory/memory.py stale` and re-verify anything it lists.
